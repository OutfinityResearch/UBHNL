...
### Configuration surface (normative)
Instantiation policy must be configurable via session/query options (DS-009), not hard-coded.

Conceptual configuration object:
```
SchemaPolicy = {
  mode: "minimal" | "batch",
  maxInstancesPerSchemaPerRound: number,   // e.g., 100
  maxRounds: number,                       // e.g., 50
  maxTotalInstances: number,               // e.g., 10_000 (safety cap)
  scanOrder: "stable" | "heuristic"        // stable is required for determinism
}
```
